...
- 支持无限嵌套深度的网络
- 支持全展开 2D 卷积算子构建与部署，`padding` 不支持
...
- 支持全展开 1D 卷积算子构建与部署，`padding` 不支持
...
- 支持全展开 1D/2D 转置卷积算子。所有的卷积算子均支持 `padding`
- 新增直接编码器
- 修复后端的一些错误
...
- 移除 Python 3.9 支持
- 支持 uv 构建
- 提高 `paicorelib` 依赖版本至 `>=1.5.1`、`<1.6.0`
- 支持在线核及在线学习算子的构建与部署
- 初步支持 tiling 后端优化
