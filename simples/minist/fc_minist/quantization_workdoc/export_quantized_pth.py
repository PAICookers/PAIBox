"""
LeNet-5 MNIST 量化工作文档示例。

功能：
    1. 读取现有的 FP32 best_model.pth
    2. 使用 FX prepare_fx 插入 Observer 并做校准
    3. 使用 convert_fx_to_manual 得到 #sym:quantized_model
    4. 导出校准数据 JSON
    5. 导出量化后每层的输入 / 输出 / 权重 scale 和 zp
    6. 导出量化之后的计算图 TXT
    7. 将量化模型的 state_dict 导出为 pth
"""

import json
import os
import sys
import torch
import torch.nn as nn
import torchvision
import torchvision.transforms as transforms
from torch.utils.data import DataLoader
from torch.ao.quantization import QConfig, MinMaxObserver, get_default_qconfig_mapping
from torch.ao.quantization.backend_config import get_qnnpack_backend_config
from torch.ao.quantization.quantize_fx import fuse_fx, prepare_fx

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "..", ".."))
for path in (REPO_ROOT, BASE_DIR):
    if path not in sys.path:
        sys.path.insert(0, path)

from simples.quantize_tools import (
    convert_fx_to_manual,
    export_quantized_model_summary,
)

from model import LeNet5
from simples.res_test.model import ResNetMNIST


DATA_DIR = os.path.join(REPO_ROOT, "simples","data")
SOURCE_CKPT = os.path.join(
    REPO_ROOT, "simples",  "res_test", "checkpoints", "best_model.pth"
)
OUTPUT_DIR = os.path.join(BASE_DIR, "outputs")
CALIB_DIR = os.path.join(OUTPUT_DIR, "calibration")
CALIB_SAMPLES_PTH = os.path.join(CALIB_DIR, "calibration_samples.pt")
CALIB_STATS_JSON = os.path.join(CALIB_DIR, "calibration_stats.json")
MODEL_SUMMARY_JSON = os.path.join(OUTPUT_DIR, "quantized_model_layers.json")
MODEL_GRAPH_TXT = os.path.join(OUTPUT_DIR, "quantized_model_graph.txt")
OUTPUT_PTH = os.path.join(OUTPUT_DIR, "quantized_resnet_mnist_state_dict.pth")


def build_calibration_loader(
    data_dir: str,
    batch_size: int = 256,
    num_workers: int = 2,
) -> DataLoader:
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.1307,), (0.3081,)),
    ])

    dataset = torchvision.datasets.MNIST(
        root=data_dir,
        train=True,
        download=True,
        transform=transform,
    )

    return DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=True,
        num_workers=num_workers,
        pin_memory=True,
    )


def get_quantization_config():
    my_qconfig = QConfig(
        activation=MinMaxObserver.with_args(
            dtype=torch.qint8,
            qscheme=torch.per_tensor_symmetric,
        ),
        weight=MinMaxObserver.with_args(
            dtype=torch.qint8,
            qscheme=torch.per_tensor_symmetric,
        ),
    )

    qconfig_mapping = get_default_qconfig_mapping("qnnpack")
    qconfig_mapping.set_object_type(nn.Conv2d, my_qconfig)
    qconfig_mapping.set_object_type(nn.Linear, my_qconfig)
    qconfig_mapping.set_object_type(nn.ReLU, my_qconfig)
    return qconfig_mapping


def export_calibration_data(prepared_model, calibration_batches):
    os.makedirs(CALIB_DIR, exist_ok=True)

    # 保存实际参与校准的 batch，便于后续复现实验。
    torch.save(calibration_batches, CALIB_SAMPLES_PTH)

    # 保存 observer 统计量，便于查看每层的量化参数。
    records = {}
    for name, module in prepared_model.named_modules():
        if hasattr(module, "calculate_qparams"):
            scale, zero_point = module.calculate_qparams()

            if torch.is_tensor(scale) and scale.numel() == 1:
                scale = scale.item()
            if torch.is_tensor(zero_point) and zero_point.numel() == 1:
                zero_point = zero_point.item()

            item = {
                "scale": float(scale),
                "zero_point": int(zero_point),
            }

            if hasattr(module, "min_val") and hasattr(module, "max_val"):
                min_val = module.min_val
                max_val = module.max_val
                if torch.is_tensor(min_val) and min_val.numel() == 1:
                    item["min_val"] = float(min_val.item())
                if torch.is_tensor(max_val) and max_val.numel() == 1:
                    item["max_val"] = float(max_val.item())

            records[name] = item

    with open(CALIB_STATS_JSON, "w", encoding="utf-8") as f:
        json.dump(records, f, ensure_ascii=False, indent=2)


def load_fp32_model() -> ResNetMNIST:
    if not os.path.exists(SOURCE_CKPT):
        raise FileNotFoundError(f"找不到预训练文件: {SOURCE_CKPT}")

    model = ResNetMNIST(num_classes=10)
    model.load_state_dict(torch.load(SOURCE_CKPT, map_location="cpu"))
    model.eval()
    return model


def main() -> None:
    device = torch.device("cpu")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CALIB_DIR, exist_ok=True)

    print("=== ResNet-18 FX 量化并导出 pth ===")
    print(f"[1] 读取 FP32 权重: {SOURCE_CKPT}")
    fp32_model = load_fp32_model().to(device)

    print("[2] 构建量化配置和示例输入")
    qconfig_mapping = get_quantization_config()
    example_inputs = torch.randn(1, 1, 28, 28).to(device)

    print("[3] 融合模型并插入 Observer")
    fused_model = fuse_fx(fp32_model)
    prepared_model = prepare_fx(
        fused_model,
        qconfig_mapping,
        (example_inputs,),
        backend_config=get_qnnpack_backend_config(),
    )

    print("[4] 使用训练集前几个 batch 做校准")
    calib_loader = build_calibration_loader(DATA_DIR)
    calib_batches = 10
    calibration_batches = []
    with torch.no_grad():
        for i, (images, _) in enumerate(calib_loader):
            if i >= calib_batches:
                break
            calibration_batches.append(images.cpu())
            prepared_model(images.to(device))

    print("[4-1] 导出校准样本和 observer 统计")
    export_calibration_data(prepared_model, calibration_batches)

    print("[5] 转换为量化模型")
    quantized_model = convert_fx_to_manual(prepared_model)

    print("[5-1] 导出量化层参数 JSON 和计算图 TXT")
    export_quantized_model_summary(
        quantized_model,
        MODEL_SUMMARY_JSON,
        MODEL_GRAPH_TXT,
    )
    print(f"[完成] 量化层参数已保存到: {MODEL_SUMMARY_JSON}")
    print(f"[完成] 量化计算图已保存到: {MODEL_GRAPH_TXT}")

    print("[6] 导出量化模型参数 pth")
    torch.save(quantized_model.state_dict(), OUTPUT_PTH)
    print(f"[完成] 量化参数已保存到: {OUTPUT_PTH}")


if __name__ == "__main__":
    main()
