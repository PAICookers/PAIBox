...
from simples.res_cifar10.model18 import ResNet18CIFAR10
import os
import sys
import torch
...
def get_quantization_config(symmetric=True):
    ...
    # 保持与旧版 Eager 模式相同的行为：让池化和展平等纯函数透明透传（不插入观察器或改变 scale）
    qconfig_mapping.set_object_type(torch.max_pool2d, None)
    qconfig_mapping.set_object_type(torch.flatten, None)
    qconfig_mapping.set_object_type(torch.nn.functional.max_pool2d, None)
    qconfig_mapping.set_object_type(torch.nn.functional.avg_pool2d, None)
    qconfig_mapping.set_object_type(nn.AdaptiveAvgPool2d, None)
    qconfig_mapping.set_object_type(
        torch.nn.functional.adaptive_avg_pool2d, None)

    return qconfig_mapping


def main():
    print(f"=== ResNet18CIFAR10 自动量化 ===")
    ...
    model_path = os.path.join(BASE_DIR, "checkpoints", "best_model18.pth")
    if not os.path.exists(model_path):
        print(f"警告：找不到预训练文件 {model_path}，使用随机初始化模型仅作测试展示。")
        fp32_model = ResNet18CIFAR10(num_classes=10)
    else:
        fp32_model = ResNet18CIFAR10(num_classes=10)
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(train_loader):
            if i >= num_calib_batches:
                break
            prepared_model(data.to(device))

    print("\n[4] 正在将带 Observer 的 FX 模型转化为完全离线的自动 ManualQuant 模型...")
    manual_model = convert_fx_to_manual(
        prepared_model, use_lut=False, activation_symmetric=symmetric)

    propagate_tensor_shape(manual_model, example_inputs)

    print("\n[5] 正在评估合并后的定点网络精度...")
    _, int8_acc = evaluate(manual_model, test_loader, criterion, device)

    print("\n" + "=" * 50)
    print("===> 最终精度对比报告:")
    print(f"     FP32 (高精度):    {fp32_acc:.2f}%")
    print(f"     Symmetric Int8: {int8_acc:.2f}%")
    print("=" * 50)

    print("\n[6] 导出 Int8 权重及量化参数...")
    export_dir = os.path.join(BASE_DIR, "exported_params18_symmetric")
