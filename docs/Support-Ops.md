...
芯片具备两类计算核：离线推理核（简称离线核）、在线学习核（在线核）。其中离线核支持的神经元配置项如下表所列：
...
在线核支持的 LIF 神经元配置如下表所列：

|   支持功能   | 可写 |        取值         |           功能描述           |
| :----------: | ---- | :-----------------: | :--------------------------: |
|   复位电平   | ✅   | 6/32比特有符号数\*  |        可配置复位电平        |
| 比较前后泄露 | ✅   |        前/后        |   阈值比较发生在泄露前/后    |
|    正阈值    | ✅   | 15/32比特有符号数\* |         可配置正阈值         |
|   地板阈值   | ✅   | 7/32比特有符号数\*  | 地板阈值，膜电平不会低于此值 |
|   泄露电平   | ✅   | 15/32比特有符号数\* |        可配置泄露幅值        |
|  初始膜电平  | ✅   | 6/32比特有符号数\*  | 每次开始帧到来时膜电平复位值 |
|    膜电平    | ❌   | 15/32比特有符号数\* |        当前时刻膜电平        |

\*参数取值范围根据在线核设置的权重位宽有所不同，当其为1-bit、2-/4-/8-bit 权重位宽时，取值分别为前、后所示值。

## 突触

芯片不支持Alpha、AMBA、GABA等类型突触，仅支持 STDP 突触。
