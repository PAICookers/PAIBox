...
def get_args() -> argparse.Namespace:
    """解析命令行参数。"""
    parser = argparse.ArgumentParser(description="CIFAR-10 SimpleVGG 训练")
    parser.add_argument("--epochs",       type=int,
                        default=60,   help="训练轮数")
    parser.add_argument("--batch-size",   type=int,
                        default=512,  help="批大小")
...
def main() -> None:
    ...
    print(
        f"[模型] SimpleVGG 参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
