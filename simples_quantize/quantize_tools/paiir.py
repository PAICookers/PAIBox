...
from paibox.paiir.ir.quantized_ops import (
...
def _bind_quantized_params(
    ...
    module.register_buffer("weight_int8", weight_q.to(weight_param.device).clone())

    with torch.no_grad():
        weight_param.copy_(weight_q.to(weight_param.device, weight_param.dtype))

    if bias_fp32 is None:
        return

    bias_q = torch.round(bias_fp32.detach() / accum_scale).to(torch.int32)
    module.register_buffer("bias_int32", bias_q.to(weight_param.device).clone())
...
def _make_conv2d_like(module: ManualQuantConv2d | ManualQuantConvReLU2d) -> nn.Conv2d:
    ...
    _bind_quantized_params(conv, module.weight_q, module.bias_val, module.s_in * module.s_w)
    return conv
...
def _map_manual_conv_relu(module: ManualQuantConvReLU2d) -> QuantizedSequentialOp:
    return QuantizedSequentialOp(
        _make_conv2d_like(module),
        _relu_act(module.s_in, module.s_w, module.s_out, module.activation_symmetric),
    )


def _map_manual_conv(module: ManualQuantConv2d) -> QuantizedSequentialOp:
    return QuantizedSequentialOp(
        _make_conv2d_like(module),
...
def _map_manual_linear_relu(module: ManualQuantLinearReLU) -> QuantizedSequentialOp:
    return QuantizedSequentialOp(
...
def _map_manual_linear(module: ManualQuantLinear) -> QuantizedSequentialOp:
    return QuantizedSequentialOp(
        _make_linear_like(module),
        _linear_act(module.s_in, module.s_w, module.s_out),
    )
...
def _map_manual_conv_add_relu(
    ...
    return QuantizedConvAddReLU2dOp(
        conv=_make_conv2d_like(module.conv),
        act=_relu_act(
            module.conv.s_in,
            module.conv.s_w,
            module.out_scale,
...
def register_manual_quantized_paiir() -> None:
    ...
    _mark_leaf_modules()
    _register_once(ManualQuantConvReLU2d, _map_manual_conv_relu)
    _register_once(ManualQuantConv2d, _map_manual_conv)
    _register_once(ManualQuantLinearReLU, _map_manual_linear_relu)
    _register_once(ManualQuantLinear, _map_manual_linear)
