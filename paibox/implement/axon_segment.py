...
def get_axon_segments(
    axons: Sequence[Union[NeuDyn, InputProj]], lcn_ex: LCN_EX
) -> List[List[AxonSegment]]:
    ...
    for axon in axons:
        ...
        for i in range(num_tick):
            ...
            axon_segments.append(
                AxonSegment(
                    slice(start_index, end_index, 1), i, slice(start_addr, end_addr, 1)
                )
            )
