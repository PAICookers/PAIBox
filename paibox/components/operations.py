...
class Transpose(VirtualNode):
    def __init__(
        self, source: Union[Neuron, InputProj], axis: tuple[int, ...] = (1, 0)
    ):
...
class Concat(VirtualNode):
    def __init__(self, sources: list[Union[Neuron, InputProj]], axis: int = 0):
        ...
        for source in sources:
            shapes_without_axis.append(
                source.shape_out[:axis] + source.shape_out[axis + 1 :]
            )
        ...
        self.weight_order = self.in_order.argsort()
        self._shape_out = (
            sources[0].shape_out[:axis]
            + (axis_length,)
            + sources[0].shape_out[axis + 1 :]
        )
