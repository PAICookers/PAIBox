...
def separate_network(
    nodes: Dict[str, Union[NeuDyn, InputProj]], edges: Dict[str, SynSys]
    ...
    while nodes_remaining:
        ...
        component_edges = {
            edge_name: edge
            for edge_name, edge in edges.items()
            if edge.source.name in component_nodes and edge.dest.name in component_nodes
        }
        component = (
            {node_name: nodes[node_name] for node_name in component_nodes},
            component_edges,
        )
