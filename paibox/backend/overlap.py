...
from .types import InputProj, Neuron, NodeSliceType, NodeType
...
def LL_overlap(
    node_list1: list[NodeSliceType], node_list2: list[NodeSliceType]
) -> bool:
...
def NL_overlap(
    node: Union[NodeType, NodeSliceType], node_list: list[NodeSliceType]
) -> bool:
...
def NN_cover(part_node_slice: NodeSliceType, whole_node_slice: NodeSliceType) -> bool:
    return part_node_slice.target == whole_node_slice.target and cover(
        part_node_slice.index, whole_node_slice.index
    )
